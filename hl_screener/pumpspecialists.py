...
from .pumpfun import FEE, LAMPORTS, TX_COST_SOL, _state_before, _value, connect, copy_trade, get_meta
...
WITH pos AS (
  ...
  FROM trades WHERE +mint IN (SELECT mint FROM mature_cand) GROUP BY wallet, mint)
SELECT p.wallet, p.mint, f.slot, f.ts, f.vsol, f.vtok, p.cost, p.back, p.tin, p.tout, m.ts
FROM pos p JOIN trades f ON f.rowid = p.fbr JOIN mints m ON m.id = p.mint
...
def positions(c: sqlite3.Connection, min_mcap: float) -> list[dict[str, Any]]:
    """Every position whose first buy came at `min_mcap` SOL of market cap or more, with its own result in SOL."""
    try:
        rows = c.execute(POS_SQL, (min_mcap,)).fetchall()
    except sqlite3.OperationalError:
        return []                                            # before pumpmature made its candidate table
    last: dict[int, tuple[tuple[int, int] | None, float]] = {}   # mint -> its last state and the fee paid then
    out = []
    for wallet, mint, slot, ts, vsol, vtok, cost, back, tin, tout, mts in rows:
        if mint not in last:
            last[mint] = (_state_before(c, mint, None), _fee_known(c, mint, None))
        (state, fee), held = last[mint], max(0, tin - tout)
        got = back * min(1.0, tin / tout) if tout > tin else back
        worth = _value(state, held, fee) * LAMPORTS if held and state else 0.0
        out.append({"wallet": wallet, "mint": mint, "slot": slot, "ts": ts, "mcap": vsol / vtok * 1e6, "cost": cost / LAMPORTS,
                    "pnl": (got + worth - cost) / LAMPORTS, "held": held > 0.02 * tin, "age_s": ts - mts})
    return out


def _detail(c: sqlite3.Connection, p: dict[str, Any], latency: int, stake: float, tx: float) -> dict[str, Any]:
    """A position up close: when the wallet first sold, what a copy made, and whether the coin was a bundle."""
    mint, wallet, fb = p["mint"], p["wallet"], p["slot"]
...
def specialists(db_path: str | Path, min_mcap: float = MC_LEVELS["mc100k"], min_coins: int = 10, detail_top: int = 40,
    ...
    coins that made the most, up close and copied, best copy first. A wallet `passes` when it made money in both
    halves of the window, a copy did too, and its two best coins are under half of what it won."""
    t_build = time.time()
    ...
    try:
        ...
        mid = statistics.median(p["ts"] for p in ps)          # halves by entry time, equal in size
        buckets = []
        for i, (name, lo) in enumerate(BUCKETS):
            hi = BUCKETS[i + 1][1] if i + 1 < len(BUCKETS) else float("inf")
            sub = [p for p in ps if lo <= p["mcap"] < hi]
            if sub:
                buckets.append({"bucket": name, "n": len(sub), "roi": _roi(sub), "win_rate": sum(p["pnl"] > 0 for p in sub) / len(sub)})
        by: dict[int, list[dict[str, Any]]] = {}
        for p in ps:
            by.setdefault(p["wallet"], []).append(p)
        many = {w: v for w, v in by.items() if len(v) >= min_coins}
        both = [w for w, v in many.items() if (_roi([p for p in v if p["ts"] < mid]) or 0) > 0
                and (_roi([p for p in v if p["ts"] >= mid]) or 0) > 0]
        rows = []
        for w in sorted(many, key=lambda w: sum(p["pnl"] for p in many[w]), reverse=True)[:detail_top]:
            v = many[w]
            close = sorted(v, key=lambda p: p["ts"])[-DETAIL_MAX:]
            for p in close:
                p.update(_detail(c, p, latency, stake, tx))
            copied = [dict(p, copy_cost=stake) for p in close if p["copy"] is not None]
            wins = sorted((p["pnl"] for p in v if p["pnl"] > 0), reverse=True)
            holds = [p["hold_s"] for p in close if p["hold_s"] is not None]
            bought = c.execute("SELECT COUNT(DISTINCT mint) FROM trades WHERE wallet = ? AND buy = 1", (w,)).fetchone()[0]
            r = {"addr": c.execute("SELECT addr FROM wallets WHERE id = ?", (w,)).fetchone()[0], "n": len(v),
                 "share": len(v) / bought if bought else None, "cost_sol": sum(p["cost"] for p in v),
                 "pnl_sol": sum(p["pnl"] for p in v), "roi": _roi(v), "roi_h1": _roi([p for p in v if p["ts"] < mid]),
                 "roi_h2": _roi([p for p in v if p["ts"] >= mid]), "win_rate": sum(p["pnl"] > 0 for p in v) / len(v),
                 "top2_share": sum(wins[:2]) / sum(wins) if wins else None,
                 "mcap_med_usd": statistics.median(p["mcap"] for p in v) * SOL_USD,
                 "hold_med_min": statistics.median(holds) / 60 if holds else None,
                 "bundle_share": sum(p["bundle"] for p in close) / len(close), "held_share": sum(p["held"] for p in v) / len(v),
                 "copy_n": len(copied), "copy_roi": _roi(copied, "copy", "copy_cost"),
                 "copy_roi_h1": _roi([p for p in copied if p["ts"] < mid], "copy", "copy_cost"),
                 "copy_roi_h2": _roi([p for p in copied if p["ts"] >= mid], "copy", "copy_cost"),
                 "copy_win_rate": sum(p["copy"] > 0 for p in copied) / len(copied) if copied else None}
            r["passes"] = bool((r["roi_h1"] or 0) > 0 and (r["roi_h2"] or 0) > 0 and (r["copy_roi_h1"] or 0) > 0
                               and (r["copy_roi_h2"] or 0) > 0 and (r["top2_share"] or 1) <= 0.5)
            rows.append(r)
        ...
        return {"generated": int(time.time()), "build_s": round(time.time() - t_build, 1), "params": params,
                "counts": {"positions": len(ps), "wallets": len(by), "wallets_min": len(many), "both_halves": len(both),
                           "profitable": sum(1 for v in many.values() if sum(p["pnl"] for p in v) > 0)},
...
def log_lines(rep: dict[str, Any], top: int = 10) -> list[str]:
    ...
    for i, r in enumerate(rep["wallets"][:top], 1):
        ...
        lines.append(f"specialist {i} {r['addr']}: {r['n']} coins ({r['share']:.0%} of what it buys), own {pct(r['roi'], 1)} "
                     ...
                     f"{r['bundle_share']:.0%} bundles, {r['held_share']:.0%} still held{', PASSES' if r['passes'] else ''}")
    return lines
